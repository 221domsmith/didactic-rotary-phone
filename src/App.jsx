...
const getScoreLabel = (s) => {
  if (s <= 30) return { label: "Critical", color: "#ef4444", bg: "#1f0a0a", desc: "Your revenue strategy has significant gaps. You're likely leaving 25–40% of potential revenue uncaptured every year." };
  if (s <= 55) return { label: "Developing", color: "#f97316", bg: "#1f0f05", desc: "You have some pricing awareness but no systematic strategy. Structured RM could move you 15–25% higher." };
  if (s <= 75) return { label: "Functional", color: "#eab308", bg: "#1a1500", desc: "You're doing better than most, but there are clear gaps in your peak and off-peak strategy." };
  return { label: "Optimized", color: "#22c55e", bg: "#051a0a", desc: "Strong foundation. Fine-tuning your channel mix and demand forecasting could push you to the next tier." };
};
...
export default function PeakrateApp() {
  ...
  const c = {
    wrap: { minHeight: "100vh", background: "#080807", color: "#e8e2d8", fontFamily: "'Georgia','Times New Roman',serif", display: "flex", flexDirection: "column", alignItems: "center", padding: "0 20px 80px" },
    hdr: { width: "100%", maxWidth: 660, padding: "32px 0 0", display: "flex", alignItems: "center", justifyContent: "space-between" },
    logo: { fontSize: 13, letterSpacing: "0.3em", color: "#c8a84b", textTransform: "uppercase", cursor: "pointer" },
    ...
    lbl: { fontSize: 11, color: "#c8a84b", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 10 },
    h1: { fontSize: 36, fontWeight: 400, lineHeight: 1.2, color: "#f0e8d8", marginBottom: 14, letterSpacing: "-0.02em" },
    sub: { fontSize: 15, color: "#6a5e4e", lineHeight: 1.75, marginBottom: 36, maxWidth: 500 },
    gold: { width: 44, height: 2, background: "#c8a84b", marginBottom: 30 },
    bizBtn: (id) => ({ width: "100%", background: hovBiz === id ? "#141209" : "#0e0d0b", border: `1px solid ${hovBiz === id ? "#c8a84b" : "#232018"}`, borderRadius: 3, padding: "18px 22px", display: "flex", alignItems: "center", gap: 14, cursor: "pointer", marginBottom: 10, transition: "all 0.18s", textAlign: "left" }),
    optBtn: (o) => ({ width: "100%", background: hovOpt === o ? "#141209" : "#0e0d0b", border: `1px solid ${hovOpt === o ? "#c8a84b" : "#232018"}`, borderRadius: 3, padding: "13px 18px", color: hovOpt === o ? "#f0e8d8" : "#b0a490", fontSize: 14, cursor: "pointer", marginBottom: 9, textAlign: "left", fontFamily: "Georgia,serif", transition: "all 0.15s" }),
    prog: { width: "100%", height: 2, background: "#181614", borderRadius: 1, marginBottom: 38, overflow: "hidden" },
    progFill: { height: "100%", background: "#c8a84b", width: `${progress}%`, transition: "width 0.3s ease", borderRadius: 1 },
    ...
    leakBox: { background: "#0c0b09", border: "1px solid #232018", borderLeft: "3px solid #c8a84b", padding: "20px 24px", borderRadius: 2, marginBottom: 22 },
    aiBox: { background: "#0c0b09", border: "1px solid #232018", padding: "18px 22px", borderRadius: 2, marginBottom: 22, position: "relative" },
    recItem: (ev) => ({ background: ev ? "#090b08" : "#0c0b09", border: `1px solid ${ev ? "#283020" : "#1c1a16"}`, padding: "15px 18px", borderRadius: 2, marginBottom: 8, display: "flex", gap: 12, alignItems: "flex-start" }),
    inp: { width: "100%", background: "#0c0b09", border: "1px solid #232018", borderRadius: 2, padding: "12px 14px", color: "#d4c4a0", fontSize: 14, fontFamily: "Georgia,serif", outline: "none", boxSizing: "border-box", marginBottom: 10 },
    pBtn: { background: "#c8a84b", color: "#080807", border: "none", borderRadius: 2, padding: "14px 28px", fontSize: 13, letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer", fontFamily: "Georgia,serif", fontWeight: 700, width: "100%", marginBottom: 9 },
    sBtn: { background: "transparent", color: "#c8a84b", border: "1px solid #c8a84b", borderRadius: 2, padding: "13px 28px", fontSize: 13, letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer", fontFamily: "Georgia,serif", width: "100%", marginBottom: 9 },
    div: { width: "100%", height: 1, background: "#181614", margin: "26px 0" },
    back: { background: "none", border: "none", color: "#3a3428", fontSize: 12, cursor: "pointer", letterSpacing: "0.1em", textTransform: "uppercase", fontFamily: "Georgia,serif", padding: 0 },
    ok: { background: "#051a0a", border: "1px solid #22c55e", borderRadius: 2, padding: "16px 20px", marginTop: 8, fontSize: 13, color: "#86efac", lineHeight: 1.65 },
  };
  ...
  return (
    <div style={c.wrap}>
      <div style={c.hdr}>
        ...
        <button
          onMouseEnter={() => setHovBTS(true)}
          onMouseLeave={() => setHovBTS(false)}
          onClick={() => go(() => setStep("why"))}
          style={{ background: "none", border: "none", cursor: "pointer", fontSize: 12, letterSpacing: "0.15em", textTransform: "uppercase", fontFamily: "Georgia,serif", color: hovBTS ? "#c8a84b" : "#3a3428", transition: "color 0.2s", padding: 0 }}
        >
          Why Peakrate?
        </button>
      </div>
        ...
        {step === "landing" && (
          <div>
            ...
            {BUSINESS_TYPES.map((bt) => (
              <button
                key={bt.id}
                style={c.bizBtn(bt.id)}
                onMouseEnter={() => setHovBiz(bt.id)}
                onMouseLeave={() => setHovBiz(null)}
                onClick={() => selectBiz(bt.id)}
              >
                <span style={{ fontSize: 26 }}>{bt.icon}</span>
                <span style={{ fontSize: 15, color: "#d4c4a0" }}>{bt.label}</span>
                <span style={{ marginLeft: "auto", color: "#3a3428", fontSize: 16 }}>→</span>
              </button>
            ))}
            <div style={{ marginTop: 36, paddingTop: 20, borderTop: "1px solid #161410", fontSize: 12, color: "#2a2620", lineHeight: 1.7 }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                {isEventQ && (
                  <div style={{ fontSize: 10, color: "#c8a84b", letterSpacing: "0.15em", textTransform: "uppercase", background: "#1a1500", padding: "3px 8px", borderRadius: 2 }}>
                    Event Intelligence
                  </div>
                )}
                <div style={{ fontSize: 12, color: "#3a3428" }}>{currentQ + 1} / {questions.length}</div>
              </div>
            </div>
            <div style={c.prog}><div style={c.progFill} /></div>

            {currentQ === baseQCount && (
              <div style={{ background: "#0e0d0b", border: "1px solid #2a2520", borderLeft: "3px solid #c8a84b", padding: "14px 18px", borderRadius: 2, marginBottom: 26 }}>
                <div style={{ fontSize: 10, color: "#c8a84b", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 5 }}>Now: Event Demand Analysis</div>
                <div style={{ fontSize: 13, color: "#5a5040", lineHeight: 1.65 }}>One-time events can inflate your historical data and distort your forecast for years. These questions identify whether you're pricing around local demand or being misled by it.</div>
              </div>
            )}

            <div style={c.lbl}>{isEventQ ? "Event Demand Intelligence" : `${typeLabel} · Revenue Audit`}</div>
            <div style={{ fontSize: 22, color: "#f0e8d8", fontWeight: 400, lineHeight: 1.4, marginBottom: 28 }}>{questions[currentQ].label}</div>
            {questions[currentQ].options.map((opt) => (
              <button
                ...
                onClick={() => answer(questions[currentQ].id, opt)}
              >
                {opt}
              </button>
            ))}
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 24, marginBottom: 22 }}>
              <div style={c.circle}>
                ...
                <div style={{ fontSize: 11, color: "#3a3428", marginTop: 2 }}>/ 100</div>
              </div>
              <div>
            ...
            <div style={c.leakBox}>
              <div style={{ fontSize: 11, color: "#c8a84b", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 6 }}>Estimated Annual Revenue Gap</div>
              <div style={{ fontSize: 34, color: "#f0e8d8", fontWeight: 300 }}>
              ...
              <div style={{ fontSize: 12, color: "#4a4438", marginTop: 4, fontStyle: "italic" }}>Based on your current strategy vs. optimized RM benchmarks for your business type</div>
              {answers.events_onetime?.includes("assumed") && (
                <div style={{ marginTop: 12, padding: "10px 14px", background: "#1f0a0a", border: "1px solid #7f1d1d", borderRadius: 2, fontSize: 12, color: "#fca5a5", lineHeight: 1.6 }}>
            ...
            <div style={c.aiBox}>
              <div style={{ fontSize: 10, color: "#3a3428", letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 10 }}>Consultant's Read</div>
              {loadingAI
                ? <div style={{ fontSize: 13, color: "#3a3428", fontStyle: "italic" }}>Analyzing your results...</div>
                : <div style={{ fontSize: 14, color: "#a09080", lineHeight: 1.8, fontStyle: "italic" }}>{aiInsight || "Your answers reveal clear, addressable gaps in your revenue strategy the kind that compound quietly over years without a structured RM approach."}</div>
              }
              <div style={{ position: "absolute", bottom: 12, right: 16, fontSize: 10, color: "#2a2620" }}>Dom Smith · Peakrate</div>
            </div>

            <div style={c.lbl}>Recommendations</div>
            {recs.map((rec, i) => (
              <div key={i} style={c.recItem(rec.event)}>
                <div style={{ fontSize: 17, color: rec.event ? "#2a3518" : "#222018", fontWeight: 700, minWidth: 22, paddingTop: 2 }}>0{i + 1}</div>
                <div style={{ fontSize: 13, color: rec.event ? "#7a9060" : "#7a6e5e", lineHeight: 1.7, flex: 1 }}>{rec.text}</div>
                {rec.event && <div style={{ fontSize: 10, color: "#4a5830", paddingTop: 2, flexShrink: 0 }}>📅 Event RM</div>}
              </div>
            ))}
            ...
            {!reportDone && !consultDone ? (
              <div>
                ...
                <p style={{ fontSize: 14, color: "#5a5040", lineHeight: 1.75, marginBottom: 22 }}>Get your full report emailed to you, or book a free 30-minute consultation with Dom to walk through your results and build a 90-day action plan.</p>
        ...
        {step === "why" && (
          <div>
            ...
            <div style={{ marginTop: 36 }}>
              ...
              <div style={c.gold} />
              <div style={{ fontSize: 15, color: "#7a6e5e", lineHeight: 1.85, marginBottom: 24 }}>
              ...
              <div style={{ fontSize: 15, color: "#7a6e5e", lineHeight: 1.85, marginBottom: 36 }}>
              ...
              <div style={{ background: "#0c0b09", border: "1px solid #232018", borderLeft: "3px solid #c8a84b", padding: "24px 28px", borderRadius: 2, marginBottom: 32 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 20 }}>
                  {[
                    ...
                    <div key={i}>
                      ...
                      <div style={{ fontSize: 13, color: "#c8a84b", marginBottom: 3, letterSpacing: "0.05em" }}>{item.org}</div>
                      <div style={{ fontSize: 12, color: "#d4c4a0", marginBottom: 6 }}>{item.role}</div>
                      <div style={{ fontSize: 12, color: "#4a4438", lineHeight: 1.6, fontStyle: "italic" }}>{item.note}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 32 }}>
                {[
                  ...
                  <div key={i} style={{ background: i === 1 ? "#0a0e09" : "#0c0b09", border: i === 1 ? "1px solid #2a3820" : "1px solid #1c1a16", padding: "20px", borderRadius: 2 }}>
                    <div style={{ fontSize: 13, color: i === 1 ? "#7a9860" : "#5a5040", letterSpacing: "0.05em", marginBottom: 14, fontWeight: 600 }}>{col.icon} {col.label}</div>
                    {col.points.map((p, j) => (
                      <div key={j} style={{ fontSize: 13, color: i === 1 ? "#8a9870" : "#4a4438", lineHeight: 1.5, marginBottom: 8, paddingLeft: 4 }}>{p}</div>
                    ))}
                  </div>
                ))}
              </div>

              <div style={{ background: "#0c0b09", border: "1px solid #232018", padding: "22px 26px", borderRadius: 2, marginBottom: 32 }}>
                <div style={c.lbl}>Where This Is Going</div>
                <div style={{ fontSize: 15, color: "#6a5e4e", lineHeight: 1.85 }}>
                  Peakrate isn't a side project. The vision is a full revenue management firm a team of analysts bringing enterprise-level strategy to small and mid-size businesses across hospitality, events, and creative services. The kind of firm that makes what used to cost $100,000 accessible for $500 a month. But right now it's just me. And honestly that's a feature, not a bug you get my full attention and someone who genuinely wants to win together, not just deliver a report and disappear.
                </div>
              </div>

              <div style={{ borderTop: "1px solid #161410", paddingTop: 24 }}>
                <div style={{ fontSize: 14, color: "#5a5040", fontStyle: "italic", marginBottom: 20 }}>Dom Smith, Founder · Peakrate</div>
